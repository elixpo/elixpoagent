"""Local CLI configuration management (~/.panda/config.toml)."""
...
def get_api_key() -> str | None:
    ...
    env_key = os.environ.get("PANDA_LLM_API_KEY")
    if env_key:
        return env_key
    config = load_config()
...
def get_api_url() -> str:
    ...
    env_url = os.environ.get("PANDA_LLM_API_URL")
    if env_url:
        return env_url
    config = load_config()
...
def get_model() -> str:
    ...
    env_model = os.environ.get("PANDA_LLM_MODEL")
    if env_model:
        return env_model
    config = load_config()
